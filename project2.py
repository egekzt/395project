...
parameters_data = pd.read_excel("Part2-Scenario3.xlsx",sheet_name = "Parameters", engine="openpyxl")
...
pallet_data = pd.read_excel("Part2-Scenario3.xlsx",sheet_name = "Pallets" ) 
vehicle_data = pd.read_excel("Part2-Scenario3.xlsx",sheet_name= "Vehicles")  
order_data = pd.read_excel("Part2-Scenario3.xlsx",sheet_name = "Orders")
loading_data = pd.read_excel("Part2-Scenario3.xlsx",sheet_name ="LoadingTypes")
...
model.LOADING_TYPES = Set(initialize=loading_data["LoadingType"].unique())
...
model.capacity = Param(model.VEHICLE_TYPES,model.LOADING_TYPES,model.PALLET_SIZES,mutable=True,initialize=0)

for _, row in loading_data.iterrows():
    t = row['VehicleType']
    l = row['LoadingType']
    c1 = row['CapacityForPalletSize1']
    c2 = row['CapacityForPalletSize2']
    model.capacity[t,l,1] = c1
    model.capacity[t,l,2] = c2
...
# Corrected variables without pallet size index
model.owned_operations = Var(model.DAYS, model.VEHICLE_TYPES, model.LOADING_TYPES, domain=NonNegativeIntegers, initialize=0)
model.rented_operations = Var(model.DAYS, model.VEHICLE_TYPES, model.LOADING_TYPES, domain=NonNegativeIntegers, initialize=0)
model.operations = Var(model.DAYS, model.VEHICLE_TYPES, model.LOADING_TYPES, domain=NonNegativeIntegers, initialize=0)
...
def operations_rule(model, d, t, l):
    return model.operations[d, t, l] == model.owned_operations[d, t, l] + model.rented_operations[d, t, l]
model.OperationsLink = Constraint(model.DAYS, model.VEHICLE_TYPES, model.LOADING_TYPES, rule=operations_rule)


def operation_shipment_ruleONE(model, d):
    return sum(model.shipped_pallet[p, d] * model.pallet_size1[p] for p in model.PALLETS) <= sum(
        model.operations[d, t, l] * model.capacity[t, l, 1] 
        for t in model.VEHICLE_TYPES 
        for l in model.LOADING_TYPES
    )
...
def operation_shipment_ruleTWO(model, d):
    return sum(model.shipped_pallet[p, d] * model.pallet_size2[p] for p in model.PALLETS) <= sum(
        model.operations[d, t, l] * model.capacity[t, l, 2] 
        for t in model.VEHICLE_TYPES 
        for l in model.LOADING_TYPES
    )
...
def owned_trips_limit(model, d, t):
    return sum(model.owned_operations[d, t, l] for l in model.LOADING_TYPES) <= tripcount * model.vehicle_count[t]
...
def objective_function(model):
    owned_cost = sum(
        model.owned_operations[d, t, l] * model.vehicle_cost[t]
        for d in model.DAYS 
        for t in model.VEHICLE_TYPES 
        for l in model.LOADING_TYPES
    )
    rented_cost = sum(
        model.rented_operations[d, t, l] * model.vehicle_cost_rented[t]
        for d in model.DAYS 
        for t in model.VEHICLE_TYPES 
        for l in model.LOADING_TYPES
    )
...
try:
    obj_value = model.obj.expr()
except:
    obj_value = "Objective function not defined or no value computed."

# Dump everything to a report file
with open('model_solution_report3.txt', 'w', encoding='utf-8') as f:
    f.write('--- Solver Report ---\n\n')
    f.write(f"Runtime (seconds): {cpu_time:.4f}\n")

    # Write objective value if solved
    if hasattr(model, 'obj'):
        f.write(f"Objective Value: {model.obj.expr():.4f}\n")
    else:
        f.write("Objective Value: Not Available\n")
    ...
    for v in model.component_objects(Var, active=True):
        ...
        for index in varobject:
            f.write(f"{v.name}{index} = {varobject[index].value}\n")
